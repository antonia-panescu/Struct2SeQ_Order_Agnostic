...
class ConvTransformerEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if use_gradient_checkpoint:
            ...
            pairwise_features = pairwise_features + self.pair_dropout_out(
                checkpoint.checkpoint(
                    self.custom(self.triangle_update_out), pairwise_features, src_mask
                )
            )
            pairwise_features = pairwise_features + self.pair_dropout_in(
                checkpoint.checkpoint(
                    self.custom(self.triangle_update_in), pairwise_features, src_mask
                )
            )
        ...
        if use_gradient_checkpoint:
            pairwise_features = pairwise_features + checkpoint.checkpoint(
                self.custom(self.pair_transition), pairwise_features
            )
        else:
...
class TriangleMultiplicativeModule(nn.Module):
    ...
    def forward(self, x, src_mask):
        if src_mask is None:
            src_mask = torch.ones(*x.shape[:2]).to(x.device)
    ...
    def __init__(self, config, pretrained=False):
        ...
        if pretrained:
            self.load_state_dict(
                torch.load(
                    "../../w_pair_exps/test5/models/model0.pt", map_location="cpu"
                )
            )
